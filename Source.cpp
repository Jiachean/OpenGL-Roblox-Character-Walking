...
void keyboard(unsigned char key, int x, int y) {
    ...
    case 'j': // Start jumping
        ...
        break;
    case GLUT_ACTIVE_SHIFT + 'a':
        characterPosX += cos(cameraAngleY * M_PI / 180) * cameraSpeed * 15;
        characterPosZ -= sin(cameraAngleY * M_PI / 180) * cameraSpeed * 15;
        isRunning = true;
        break;
    case GLUT_ACTIVE_SHIFT + 's':
        characterPosX -= sin(cameraAngleY * M_PI / 180) * cameraSpeed * 15;
        characterPosZ -= cos(cameraAngleY * M_PI / 180) * cameraSpeed * 15;
        isRunning = true;
        break;
    case GLUT_ACTIVE_SHIFT + 'd':
        characterPosX -= cos(cameraAngleY * M_PI / 180) * cameraSpeed * 15;
        characterPosZ += sin(cameraAngleY * M_PI / 180) * cameraSpeed * 15;
        isRunning = true;
        break;
    case GLUT_ACTIVE_SHIFT + 'w':
        characterPosX += sin(cameraAngleY * M_PI / 180) * cameraSpeed * 15;
        characterPosZ += cos(cameraAngleY * M_PI / 180) * cameraSpeed * 15;
        isRunning = true;
        break;
...
void leftArm3()
{
    ...
    glColor3f(1, 0.5, 0.5);
    if (isRunning) {
        // Adjust arm angle for running animation
        glRotatef(walkingLegAngle, 1.0, 0.0, 0.0);
    }
    glutSolidCube(0.3);
    glPopMatrix();
...
void leftLeg3()
{
    ...
    if (isRunning) {
        // Adjust leg angle for running animation
        glRotatef(-walkingLegAngle, 1.0, 0.0, 0.0);
    }
    glutSolidCube(0.3);
    glPopMatrix();
